"""Sprint 272 - Desktop Runtime Foundation test."""
import unittest

from sam.desktop_runtime.bridge import DesktopRuntimeBridge
...
from sam.desktop_runtime.foundation.capability import DesktopCapability
from sam.desktop_runtime.foundation.contract import DesktopContract
from sam.desktop_runtime.foundation.descriptor import DesktopDescriptor
from sam.desktop_runtime.foundation.metadata import DesktopMetadata
from sam.desktop_runtime.foundation.registry import (
    DesktopRegistry,
    KNOWN_COMPONENTS,
)
...
class TestDescriptor(unittest.TestCase):
    def test_descriptor_immutable(self):
        ...
        with self.assertRaises(Exception):
            d.name = "x"  # frozen
        self.assertEqual(d.name, "desktop_runtime")
...
class TestRegistry(unittest.TestCase):
    ...
    def test_with_entry_immutable(self):
        r = DesktopRegistry().with_entry("Panels", "component")
        self.assertEqual(len(r), 1)
        self.assertEqual(r.names, ("Panels",))
        base = DesktopRegistry()
...
class TestNoIOLayers(unittest.TestCase):
    def test_forbidden_imports_absent(self):
        ...
        desktop_py = list((src / "sam" / "desktop_runtime").rglob("*.py"))
