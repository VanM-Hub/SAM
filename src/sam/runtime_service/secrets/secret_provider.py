...
# env mode produksi (M12-010: secret store REQUIRED / fail-closed)
...
def default_secret_provider() -> "SecretProvider":
    ...
    (PostgreSQL terenkripsi, M11-003).

    M12-010 (Secret Boundary Hardening):
      - `SAM_ENV=production` -> secret store WAJIB. Memakai PgSecretProvider
        dengan `strict=True` (fail-closed): master key hilang / store tak ada /
        key tak ada di store -> BLOCKED (SecretUnavailableError), TANPA fallback
        diam-diam ke env. `SAM_ENABLE_PG_SECRETS` TIDAK perlu diset (produksi
        selalu butuh store).
      - selain produksi (dev): tetap opt-in `SAM_ENABLE_PG_SECRETS=1`;
        bila tidak, env-only (perilaku default yang sudah terbukti M8/M10).
    ...
    production = os.environ.get(PRODUCTION_ENV) == PRODUCTION_VALUE
    if production or os.environ.get(PG_SECRETS_ENABLE_ENV) == "1":
        return PgSecretProvider(strict=bool(production))
