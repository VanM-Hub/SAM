...
class RuntimeCoordinator:
    ...
    async def start(self) -> RuntimeState:
        """Mulai runtime dengan recovery check + bootstrap.

        Returns:
            RuntimeState setelah startup (READY atau SAFE_MODE).
        """
        self.state = RuntimeState.INITIALIZING
        ...
        if crash_detected:
            self.telemetry.emit_event(
                event_name="startup.crash_detected",
                severity="warning",
                runtime_state="initializing",
                component="coordinator",
            )
            ...
            if not recovered:
                ...
                self.telemetry.emit_event(
                    event_name="startup.recovery_failed",
                    severity="error",
        ...
        if success:
            ...
            self.state = RuntimeState.SAFE_MODE
            self.telemetry.emit_event(
                event_name="startup.bootstrap_failed",
                severity="error",
                runtime_state="safe_mode",
                component="coordinator",
            )
    ...
    async def run(self) -> RuntimeState:
        ...
        # Mulai metrics collector di background
        import asyncio
    ...
    async def stop(self) -> RuntimeState:
        """Hentikan runtime melalui shutdown pipeline."""
        if self.state in (RuntimeState.SHUTDOWN, RuntimeState.CRASHED):
            return self.state

        self.telemetry.emit_event(
            event_name="runtime.stopping",
    ...
    async def recover(self) -> RuntimeState:
        """Pulihkan dari DEGRADED ke RUNNING via READY."""
    ...
    def adapter_name(self) -> str:
        """Nama adapter yang sedang aktif."""
