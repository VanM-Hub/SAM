# ADR-006-PREP â€” External Access Boundary Analysis

Version: 0.1.0

Status: Completed â€” Verdict A (Ready)
...
Menyiapkan ruang keputusan untuk **C-07 â€” External Access Boundaries** â€” identifikasi evidence, batas arsitektural, tanggung jawab, alternatif, dependensi tersembunyi, dan verdict readiness.
...
# Audit 1 â€” Evidence Extraction

Seluruh kalimat mengenai **External Access, Runtime Boundary, Provider, Connector, Citizen, Contract Boundary** dari dokumen otorisasi. Kutipan asli â€” tidak disimpulkan.

## 1.1 G0-001 Blueprint â€” C-07 Definition

| Line | Text (verbatim) |
|---|---|
| L160 | **C-07** â€” **Reference boundaries to external access** â€” Where the Runtime positions Providers / Connectors (external access) relative to the chain â€” Trade-off between isolation and integration ease |
| L163 | Register policy: none of C-01â€¦C-08 is decided here. Each is a candidate to be turned into a formal ADR **only** at the point an implementation-facing decision must be made, and each such ADR must not contradict the frozen baseline. |

## 1.2 SAM_ARCHITECTURE â€” Provider / Connector Role

| Line | Text (verbatim) |
|---|---|
| L50 | (implemented by Runtime, Citizens, Providers, Connectors) |
| L84 | Providers / Connectors (external access) |
| L103 | Provider / Connector â€” Implement external access / communication â€” Exercise governance |
...
## 1.3 CITIZEN_SPECIFICATION â€” Provider/Connector as Citizens
...
| L947-L957 | Citizen hierarchy: `Runtime` / `Agent` / `Provider` / `Connector` â€” all under Citizen. |

## 1.4 CONTRACT_SPECIFICATION â€” Contract as Boundary
...
## 1.5 REGISTRY_SPECIFICATION â€” Discovery Mechanism
...
## 1.6 GOVERNANCE â€” External Access Governance
...
## 1.7 R1-001 â€” Runtime Boundary & External Access

| Line | Text (verbatim) |
|---|---|
| L51 | External access â€” Providers/Connectors implement external access; Runtime does not. |
...
| L116 | (All components) â†’ Registry on every external interaction â€” the only permitted discovery mechanism (boundary: Registry reduces coupling). |
| L118 | No additional interaction is invented. The one recurrent edge â€” every external interaction goes through Registry â€” re-states the boundary mechanism, not a new behavior. |
| L196 | External-access / communication failures with the outside world â†’ Provider/Connector layer (outside Runtime). |
...
## 1.8 Root ADRs â€” C-07 Context

| Source | Line | Text (verbatim) |
|---|---|---|
| ADR-000 | L155 | Menyediakan konteks authoring yang stabil bagi ADR berikutnya (C-07, C-08, C-01, C-05). |
| ADR-000 | L175 | C-07 (Reference Boundaries to external access) & C-08 (Verification point placement): mendapat konteks authoring yang jelas (posisi relative to one Runtime) â€” tanpa mengubah validitasnya (R1-003/R2-003 Neutrality). |
...
| ADR-001 | L210 | C-07/C-08 â€” menyusul lewat lifecycle R2-001 yang sama. |
...
| ADR-005 | L256 | C-07 (External Access Boundaries): ordering tidak mempengaruhi placement Provider/Connector â€” ordering adalah internal Execution. |
| R2-002 | L101 | C-02, C-03, C-04, C-06 adalah upstream bagi C-01/C-05/C-07/C-08 â€” edges adalah konteks authoring, bukan validitas. |
...
| B-2 | **Runtime â†” External surface** | Setiap interaksi eksternal melalui Registry (discovery); komunikasi melalui Contracts. Hanya dua mekanisme cross-boundary. | R1-001 L58, L104, L116, L118 |
| B-3 | **Citizen â†” Citizen boundary** | Contract = boundary eksplisit antar Citizens. Citizens tidak discover langsung â€” melalui Registry. All Citizens obey identical constitutional rules. | CONTRACT_SPEC L55; CITIZEN_SPEC L639, L643, L647, L875 |
...
| B-5 | **Contract boundary** | Contract mendefinisikan struktur komunikasi saja â€” bukan execution, bukan lifecycle, bukan governance. | CONTRACT_SPEC L27, L55, L163, L182 |
...
| B-7 | **Citizen hierarchy** | Runtime, Agent, Provider, Connector â€” semua adalah Citizen. Runtime is not superior to Provider. No Citizen possesses constitutional privilege. | CITIZEN_SPEC L859, L863, L875, L947-L957 |
| B-8 | **Chain position** | Linear chain: Citizen â†’ Capability â†’ Registry â†’ Contract â†’ Approval â†’ Execution â†’ Audit. External access (Providers/Connectors) = outside this chain. | R1-001 L24, L104; Blueprint G0-001 component listing |

**Jumlah boundary teridentifikasi: 8.** Semua memiliki anchor dokumen eksplisit.

---

# Audit 3 â€” Architectural Responsibility
...
Tiga alternatif â€” **dievaluasi terhadap evidence aktual, bukan diciptakan.**

---

## Alternative A â€” Runtime menjadi satu-satunya gerbang akses (Runtime-mediated)

**Deskripsi:** Dari perspektif capability domain-nya, Runtime adalah satu-satunya gerbang untuk akses eksternal â€” Provider/Connector berkomunikasi dengan Runtime melalui permukaan Contracts + Registry.

### Evidence

| Evidence | What it says | Assessment |
|---|---|---|
| R1-001 L65 | "Outer surface is Contracts + Registry; everything else (strategic decision, external access, presentation, governance/authority creation) is outside." | **SUPPORTS A.** Permukaan Runtime adalah Contracts + Registry â€” hanya melalui permukaan ini akses dari luar bisa masuk ke Runtime. |
| R1-001 L116 | "(All components) â†’ Registry on every external interaction â€” the only permitted discovery mechanism." | **SUPPORTS A.** Semua interaksi eksternal melalui Registry. |
| R1-001 L58 | "The boundary between the Runtime and everything else is the two and only two mechanisms the frozen baseline permits: Contracts and Registry-based Capability discovery." | **SUPPORTS A.** Hanya dua mekanisme â€” gateway. |
| R1-001 L118 | "No additional interaction is invented." | **SUPPORTS A.** Tidak bisa membuat jalur akses baru ke komponen internal. |
| CITIZEN_SPEC L639 | "Citizens SHALL NOT discover each other directly." | **SUPPORTS A.** Discovery melalui Registry â€” akses termediasi. |
| CITIZEN_SPEC L863 | "Runtime is not superior to Provider." | **MENGOREKSI A.** Runtime bukan gateway BAGI SELURUH SAM â€” hanya gateway bagi DOMAIN-NYA. Provider juga Citizen yang sejajar. |
...
**Parsial didukung.** Evidence mendukung "Runtime-mediated access to its own domain" tetapi **tidak mendukung** "Runtime as the sole gateway for all SAM." CITIZEN_SPEC L859/L863/L875 secara eksplisit menyatakan Runtime = Citizen sejajar, bukan superior. Provider/Connector juga Citizen â€” mereka tidak memerlukan "gerbang" Runtime; mereka berinteraksi dengan Runtime sebagai Citizen-to-Citizen melalui Contracts + Registry (mekanisme universal).

---

## Alternative B â€” Komponen dapat mengakses keluar secara langsung (Direct component access)

**Deskripsi:** Komponen internal Runtime (Execution Scheduler, Approval Coordinator, dsb.) dapat mengakses Provider/Connector secara langsung â€” melewati chain Runtime.
...
| R1-001 L65 | "Outer surface is Contracts + Registry." | **BERTENTANGAN DENGAN B.** Hanya dua mekanisme â€” jalur langsung tidak diizinkan. |
...
## Alternative C â€” Boundary berbasis Contract (Contract-defined boundary)

**Deskripsi:** Posisi external access relatif terhadap chain ditentukan oleh Contract â€” Provider/Connector adalah Citizens yang berinteraksi dengan Runtime melalui Contract + Registry, persis seperti Citizen-to-Citizen lainnya. External access berada DI LUAR chain Runtime, dan boundary antara keduanya adalah Contract â€” mekanisme yang sama untuk semua interaksi antar-Citizen.

### Evidence

| Evidence | What it says | Assessment |
|---|---|---|
| CONTRACT_SPEC L55 | "A Contract makes the boundary between Citizens explicit." | **SUPPORTS C.** Contract ADALAH boundary antar Citizens â€” termasuk antara Runtime dan Provider/Connector. |
...
| CITIZEN_SPEC L639 | "Citizens SHALL NOT discover each other directly." | **SUPPORTS C.** Discovery melalui Registry â€” Contract + Registry = universal boundary. |
| CITIZEN_SPEC L643 | "Citizens SHALL publish themselves to Registry." | **SUPPORTS C.** Semua Citizens (termasuk Provider/Connector) publish ke Registry â€” boundary yang sama. |
| CITIZEN_SPEC L875 | "All Citizens obey identical constitutional rules." | **SUPPORTS C.** Tidak ada aturan khusus untuk boundary Runtimeâ†”Provider â€” aturan yang sama berlaku. |
| CITIZEN_SPEC L859 | "No Citizen possesses constitutional privilege." | **SUPPORTS C.** Tidak ada privilej khusus â€” boundary berlaku seragam. |
| CITIZEN_SPEC L863 | "Runtime is not superior to Provider." | **SUPPORTS C.** Provider tidak perlu "lewat" Runtime â€” mereka berinteraksi sebagai Citizens sejajar. |
| R1-001 L58 | "The two and only two mechanisms: Contracts and Registry-based Capability discovery." | **SUPPORTS C.** Hanya dua mekanisme â€” Contract + Registry = universal boundary. |
| R1-001 L104 | "The only cross-boundary mechanism is Contracts + Registry." | **SUPPORTS C.** Mekanisme cross-boundary universal. |
| R1-001 L51 | "External access â€” Providers/Connectors implement external access; Runtime does not." | **SUPPORTS C.** Eksternal access = luar Runtime; Contract = jembatan. |
| R1-001 L196 | "External-access / communication failures â†’ Provider/Connector layer (outside Runtime)." | **SUPPORTS C.** Provider/Connector = di LUAR Runtime; Contract = mekanisme interaksi. |
| SAM_ARCHITECTURE L103 | "Provider / Connector â€” Implement external access / communication." | **SUPPORTS C.** Provider/Connector = akses eksternal; komunikasi melalui Contract/Registry. |
...
**DIDUKUNG PENUH.** Evidence dari CONTRACT_SPEC, CITIZEN_SPEC, R1-001, SAM_ARCHITECTURE, GOVERNANCE, dan SPECIFICATION_FREEZE secara konsisten menetapkan bahwa: (1) Provider/Connector adalah Citizens yang berada di LUAR chain Runtime, (2) boundary antara Runtime dan Provider/Connector adalah Contract + Registry â€” mekanisme universal yang sama untuk semua interaksi antar-Citizen, (3) Runtime tidak punya posisi superior/privilej terhadap Provider, dan (4) tidak ada mekanisme akses selain Contracts + Registry.
...
|---|---|---|---|
| A â€” Runtime sebagai gerbang | R1-001 L58/L65/L116/L118 | CITIZEN L859/L863/L875 (Runtime = sejajar, bukan gerbang superior) | Parsial; perlu dikoreksi oleh C |
| B â€” Akses langsung komponen | â€” Tidak ada â€” | R1-001 L104/L118/L58/L65; CITIZEN L639; I5 | **Ditolak** |
| C â€” Boundary berbasis Contract | CONTRACT L55; R1-001 L58/L65/L104/L51/L116; CITIZEN L79/L639/L643/L875/L859/L863; SAM_ARCH L103; GOVERNANCE L198 | â€” Tidak ada â€” | **Didukung penuh** |
...
**Untuk ADR-006:** Alternative C ("Posisi external access di luar chain Runtime, boundary = Contract") adalah jawaban yang muncul dari evidence â€” dan ini adalah konsekuensi dari model Citizen universal yang sudah ditetapkan oleh CITIZEN_SPECIFICATION.

---

# Audit 5 â€” Hidden Dependency

Pemeriksaan: apakah C-07 bergantung pada deployment / resolution / approval / ordering / failure propagation â€” atau benar-benar independen?

| Dependensi potensial | Bergantung? | Bukti |
|---|---|---|
| **ADR-000 (Deployment Topology)** | **Tidak â€” konteks authoring saja.** ADR-000 menyediakan "konteks authoring" (R1-003/R2-003: order = strategy, bukan validity). C-07's position (di luar chain) tidak bergantung pada "satu unit deployment atau distribusi." GOVERNANCE: "valid regardless of deployment topology." | ADR-000 L155/L175 ("konteks authoring"); R2-002 L101 ("edges adalah konteks authoring, bukan validitas"); R2-003 L133 ("konteks authoring, bukan validitas") |
| **ADR-001 (Approval Decision)** | **Tidak.** GOVERNANCE L198: "Execution affecting external world should require governance approval" â€” ini constraint (constraint), BUKAN dependency. C-07 tidak perlu tahu BAGAIMANA Approval diputuskan. | ADR-001 L23 (explicitly excludes C-07 from scope) |
| **ADR-002 (Capability Resolution)** | **Tidak.** External access melalui Registry discovery â€” yang memerlukan resolusi. Tetapi KEBIJAKAN resolusi (exact-match vs compatible) tidak menentukan DI MANA Provider/Connector diposisikan. | ADR-002; C-07 = tentang posisi, bukan kebijakan resolusi |
...
| **ADR-005 (Ordering)** | **Tidak.** ADR-005 L256: "C-07: ordering tidak mempengaruhi placement Provider/Connector â€” ordering adalah internal Execution." | ADR-005 L256 |
| **R1-001 (Minimal Reference Runtime)** | **Tidak â€” konteks authoring saja.** R1-001 mendefinisikan boundary Runtime (Audit 1) â€” ini adalah FAKTA yang sudah ada, bukan dependency yang harus diselesaikan. | R1-001 L30-L65 (Audit 1 â€” Runtime Boundary) |

**Verdict: TIDAK ADA HIDDEN DEPENDENCY.** C-07 adalah keputusan atomik yang dapat diputuskan sendiri. Dependensi ke root ADR (ADR-000/001/002) adalah "konteks authoring" (R2-002 L101, R2-003 L133), bukan validitas â€” C-07 tidak menunggu root diputuskan untuk tetap valid.

---

# Audit 6 â€” Independence Test

| Kriteria | Terpenuhi? | Bukti |
|---|---|---|
| **Satu keputusan** | **Ya.** C-07 = "Where the Runtime positions Providers / Connectors relative to the chain" â€” satu pertanyaan, satu jawaban arsitektural. | G0-001 L160; seluruh evidence Audit 1 |
| **Atomik** | **Ya.** Keputusan tidak dapat dipecah menjadi sub-keputusan. "Posisi" adalah satu properti â€” tidak terpecah menjadi "internal position" + "external position" + "boundary mechanism." | R2-002 Independence Matrix: C-07 listed as single candidate |
| **Arsitektural (bukan implementasi)** | **Ya.** Pertanyaan adalah tentang posisi arsitektural (relative to chain), bukan mekanisme implementasi (API, protokol, transport, autentikasi, format data, timeout, retry policy). | Blueprint C-07 L160: "Reference boundaries" â€” "reference" = arsitektural, bukan implementasi |
...
| Membutuhkan perubahan Specification | **Tidak** | Semua evidence diambil dari spec yang sudah beku â€” tidak perlu diubah. |
...
â†’ **STOP TIDAK AKTIF.** ADR-006 dapat dilanjutkan.

---

# Catatan Penutup

Sesuai arahan Chief Architect: ini adalah ADR preparation ringan, bukan discovery baru, bukan pengulangan metodologi G1. Hasil: **C-07 adalah satu keputusan arsitektur atomik yang siap untuk ditulis sebagai ADR-006.** Evidence mendukung posisi external access di luar chain Runtime dengan Contract sebagai boundary â€” model ini adalah konsekuensi alami dari Citizen Specification yang memperlakukan Provider/Connector sebagai Citizens sejajar.
