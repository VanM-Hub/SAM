...
| Identitas rilis | **5.2.0** - Universal Governance of External Wards (M13 CERTIFIED 17/17) · **M14 (SAM Becomes Useful) closure ACCEPTED (full)** · **TIDAK bump ke 5.3.0** (verdict final Van 2026-08-15) |
...
| Status saat ini | M14 (delegated authority bridge) closure **ACCEPTED (full)** 2026-08-15 (verdict final Van); seluruh gap OpenClaw Ward real E2E ditutup via live gateway bridge (1f54822), gateway_ok=true. Jembatan health.json→runtime kini live HTTP gateway. UI v18 baseline /ui, sesuai arsitektur (halaman DEFERRED tampil jujur capability-availability); M14 operational capability ≠ REST resource (tidak ada /wards,/guardians,/providers,/credentials). |
| Branch / HEAD | `main` / `cb6c938` (M14 5.2.0; bump 5.3.0 TIDAK dieksekusi) |
...
| Tanggal update | 2026-08-15 (21:50 WITA) |
| Release | **5.2.0 (M14 CLOSED/ACCEPTED)** — stable release 5.2.0; **TIDAK ada bump 5.3.0** (verdict final Van: laporan M14 identifikasi rilis = 5.2.0; __version__ 5.2.0 sync). Draft v5.3.0 dihapus/diabaikan. |
